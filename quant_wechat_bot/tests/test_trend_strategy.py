...
class TrendStrategyTests(unittest.TestCase):
    ...
    def test_format_weekly_review_from_report_includes_candidate_pool(self) -> None:
        snapshot = trend_strategy.TrendSnapshot(
            market_label="A股",
            as_of=dt.date(2025, 1, 3),
            candidate_count=3,
            ...
            execution_plan=(
                ...
                trend_strategy.ExecutionInstruction(
                    ...
                    from_weight=0.25,
                    to_weight=0.0,
                    trigger_price=260.32,
                    stop_price=None,
                    risk_budget_pct=0.0,
                    budget_value=125000.0,
                    currency="CNY",
        ...
        report = trend_strategy.BacktestReport(
            ...
            daily_curve=(
                trend_strategy.DailyEquityPoint(dt.date(2024, 12, 27), 1.00, 1.00, 0.0),
                trend_strategy.DailyEquityPoint(dt.date(2024, 12, 30), 1.01, 1.01, 0.0),
                trend_strategy.DailyEquityPoint(dt.date(2024, 12, 31), 1.02, 1.02, 0.0),
                trend_strategy.DailyEquityPoint(dt.date(2025, 1, 1), 1.03, 1.03, 0.0),
                trend_strategy.DailyEquityPoint(dt.date(2025, 1, 2), 1.04, 1.04, 0.0),
                trend_strategy.DailyEquityPoint(dt.date(2025, 1, 3), 1.05, 1.05, 0.0),
            ),
            best_day=trend_strategy.DailyReturnSummary(dt.date(2025, 1, 2), 0.02),
            worst_day=trend_strategy.DailyReturnSummary(dt.date(2024, 12, 30), -0.01),
            exit_reason_counts=tuple(),
            periods=(
                trend_strategy.BacktestPeriod(
                    start_date=dt.date(2024, 12, 1),
                    end_date=dt.date(2025, 1, 3),
                    gross_return=0.05,
                    cost_drag=0.003,
                    portfolio_return=0.047,
                    invested_weight=0.72,
                    turnover=0.18,
                    buy_turnover=0.10,
                    sell_turnover=0.08,
                    holdings=snapshot.picks,
                    risk_on_markets=("A股",),
                    contributions=(
                        trend_strategy.PeriodContribution("603986", "兆易创新", 0.25, 0.12, 0.03),
                        trend_strategy.PeriodContribution("002384", "东山精密", 0.25, -0.04, -0.01),
                    ),
                    exit_events=(
                        trend_strategy.ExitEvent("002384", "东山精密", dt.date(2025, 1, 2), "跌破MA20", -0.04),
                    ),
                    daily_path=tuple(),
                ),
            ),
        ...
        self.assertIn("周复盘 + 下周候选池", text)
        self.assertIn("最近5个交易日", text)
        self.assertIn("下周候选池", text)
        self.assertIn("603986 兆易创新", text)
        self.assertIn("下周先处理", text)
