...
def render_weekly_review_text(market: str | None = None, top_n: int = 5, months: int = 6) -> str:
    effective_market = market if market is not None else resolve_default_market()
    return truncate_reply(
        trend_strategy.format_weekly_review(
            resolve_trend_universe_path(effective_market),
            effective_market,
            top_n=top_n,
...
def build_weekly_review_text(market: str | None = None, top_n: int = 5, months: int = 6) -> str:
    effective_market = market if market is not None else resolve_default_market()
...
def dispatch_message(message: str) -> BotReply:
    ...
    weekly_match = re.match(r"^(?:/weekly|周复盘|周报|下周候选池)(?:\s+([^\s\d]+))?(?:\s+(\d+))?(?:\s+(\d+))?$", normalized, re.I)
    if weekly_match:
        top_n = int(weekly_match.group(2) or "5")
